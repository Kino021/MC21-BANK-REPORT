...
if uploaded_file is not None:
    ...
    # Remove rows where Remark By is FGPANGANIBAN, KPILUSTRISIMO, or BLRUIZ
    df = df[~df['Remark By'].isin(['FGPANGANIBAN', 'KPILUSTRISIMO', 'BLRUIZ','MMMEJIA'])]
    
    st.write(df)
    st.write("## Summary Table")
    
    accounts = df[df['Remark Type'].isin(['Predictive', 'Follow Up'])]['Account No.'].nunique()
    connected = df[(df['Call Status'] == 'CONNECTED') & (df['Remark Type'].isin(['Predictive', 'Follow Up']))]['Account No.'].count()
    total_dialed = df[df['Remark Type'].isin(['Predictive', 'Follow Up'])]['Account No.'].count()
    penetration_rate = (total_dialed / accounts * 100) if accounts != 0 else None
    connected_rate = (connected / total_dialed * 100) if total_dialed != 0 else None
    connected_acc = df[(df['Call Status'] == 'CONNECTED') & (df['Remark Type'].isin(['Predictive', 'Follow Up']))]['Account No.'].nunique()
    ptp_acc = df[(df['Status'].str.contains('PTP', na=False)) & (df['PTP Amount'] != 0)]['Account No.'].nunique()
    ptp_rate = (ptp_acc / connected_acc * 100) if connected_acc != 0 else None
    call_drop_count = df[df['Status'] == 'DROPPED']['Account No.'].count()
